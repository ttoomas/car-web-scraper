...
// SCRAPE CAR URLS
export async function startScraping(query){
    console.log('started');

    console.log(query);

    carBazarUrl = `https://www.sauto.cz`;
...
async function scrapeCarUrls(){
    try{
        const carFullUrl = `${carBazarUrl}/${carTypeUrl}/?strana=${currPage}`;
...
async function scrapeEachCar(){
    for (const carUrl of eachCarUrl) {
        ...
        if(currentCarNumber >= carsNumber) break;
    }

    if(currentCarNumber < carsNumber){
        currPage++;
        await scrapeCarUrls();
